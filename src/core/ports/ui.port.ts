...
export interface IUI {
  ...
  askSearchSelect<T>(message: string, options: { value: T; label: string; hints?: string[] }[], searchPool?: { value: T; label: string; hints?: string[] }[]): Promise<T>;
  askSearchMultiSelect<T>(message: string, options: { value: T; label: string; hints?: string[] }[]): Promise<T[]>;
